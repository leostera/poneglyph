...
async fn cli_states_queries_and_retracts_facts_without_daemon() {
    ...
    let facts = poneglyph(
        workspace,
        &["fact", "list", "--entity", "spotify:album:2112"],
    );
    assert!(facts.contains("fact\t"));
    assert!(facts.contains("spotify:displayName"));

    let facts_json = poneglyph(
        workspace,
        &["fact", "list", "--entity", "spotify:album:2112", "--json"],
    );
    ...
    let active_facts = poneglyph(
        workspace,
        &["fact", "list", "--entity", "spotify:album:2112", "--active"],
    );
    assert!(active_facts.contains("active\t"));
    ...
    assert_eq!(active_facts.trim(), "no facts");
}
...
async fn daemon_cli_serves_status_fact_query_entity_schema_and_stop() {
    ...
    let entity_list = wait_for_output(
        workspace,
        &["entity", "list"],
        "entity\tspotify:album:signals",
    );
    assert!(entity_list.contains("spotify:album:signals"));
    ...
    let entity_search_json = poneglyph(workspace, &["entity", "search", "Signals", "--json"]);
    assert!(entity_search_json.contains("spotify:album:signals"));
    ...
    assert!(active_facts.contains("spotify:displayName"));
