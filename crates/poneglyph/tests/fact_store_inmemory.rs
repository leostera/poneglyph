...
#[tokio::test]
async fn inmemory_retractions_are_appended_and_do_not_mutate_prior_rows() {
    let store = InMemoryFactStore::new();
    assert_retractions_are_append_only(&store).await;
}

#[tokio::test]
async fn inmemory_list_facts_can_exclude_retracted_assertions_but_keep_log_visibility() {
    let store = InMemoryFactStore::new();
    assert_active_and_full_log_views(&store).await;
}

#[tokio::test]
async fn inmemory_retracting_unknown_fact_fails_the_batch() {
    let store = InMemoryFactStore::new();
    assert_invalid_retractions_fail_cleanly(&store).await;
}

#[tokio::test]
async fn inmemory_mixed_batch_rolls_back_if_any_write_is_invalid() {
    let store = InMemoryFactStore::new();
    assert_mixed_batch_rolls_back(&store).await;
}

#[tokio::test]
async fn inmemory_get_facts_returns_all_records_in_deterministic_order() {
    let store = InMemoryFactStore::new();
    assert_get_facts_returns_all_records_in_deterministic_order(&store).await;
}

#[tokio::test]
async fn inmemory_get_fact_returns_none_for_unknown_id() {
    let store = InMemoryFactStore::new();
    assert_missing_fact_returns_none(&store).await;
}

#[tokio::test]
async fn inmemory_tx_ids_are_unique_per_batch() {
    let store = InMemoryFactStore::new();
    assert_tx_ids_are_unique_per_batch(&store).await;
}

#[tokio::test]
async fn inmemory_get_facts_by_entity_uri_returns_entity_facts() {
    let store = InMemoryFactStore::new();
    assert_get_facts_by_entity_uri_returns_entity_facts(&store).await;
}

#[tokio::test]
async fn inmemory_retracting_an_already_retracted_fact_is_a_noop() {
    let store = InMemoryFactStore::new();
