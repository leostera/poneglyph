...
3. Add a background/manual compaction planner:
   ...
   - Status: the experimental planner reports an L0 segment-count compaction plan when the configurable L0 threshold is exceeded. `PONEGLYPH_LSM_L0_COMPACTION_SEGMENTS` controls the threshold, defaulting to 16 to avoid aggressive foreground compaction. The plan is exposed through `LsmFactStore::needs_compaction()`, and manual `compact()` executes planned L0 compaction into L1 when a plan exists. If no plan exists, `compact()` preserves the earlier full-store compaction behavior for tests/tools.
