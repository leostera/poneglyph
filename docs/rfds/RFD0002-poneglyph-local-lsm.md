...
3. Add a background/manual compaction planner:
   ...
   - Status: the experimental planner reports an L0 segment-count compaction plan when the configurable L0 threshold is exceeded. `PONEGLYPH_LSM_L0_COMPACTION_SEGMENTS` controls the threshold, defaulting to 16 to avoid aggressive foreground compaction. The plan is exposed through `LsmFactStore::needs_compaction()`. `compact_if_needed()` runs only planned maintenance and returns whether work happened, while `compact_in_background_if_needed()` schedules that planned work on a background thread for explicit callers. Manual `compact()` still executes planned L0 compaction first and falls back to full-store compaction for tests/tools when no plan exists.
