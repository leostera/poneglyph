...
2. Keep flush cheap:
   - `flush_memtable` only writes new L0 SSTs and appends manifest edits;
   - by default a memtable flush remains one mixed-keyspace SST, but `PONEGLYPH_LSM_SPLIT_FLUSH_BY_KEYSPACE=true` writes separate L0 SSTs by encoded keyspace (`log/tx`, `log/fact`, and each active index), giving future compaction policy column-family-like ranges without changing key encoding;
...
4. Split keyspace policies:
   ...
   - Status: planned L0 compaction now distinguishes active-index tombstones from other keyspaces. Active tombstones are dropped only when no older live segment may contain the same key; otherwise they are preserved so they continue shadowing older active assertions. Non-active tombstones are preserved by default. Opt-in split-keyspace flushing now writes disjoint L0 files per key kind, which is the first step toward compacting `active/*` ranges independently from append-only `log/*` ranges.
