...
3. Add a background/manual compaction planner:
   ...
   - Status: the experimental planner now reports an L0 segment-count compaction plan when more than four L0 segments exist. It is exposed through `LsmFactStore::needs_compaction()`, and manual `compact()` now executes that planned L0 compaction into L1 when a plan exists. If no plan exists, `compact()` preserves the earlier full-store compaction behavior for tests/tools.
